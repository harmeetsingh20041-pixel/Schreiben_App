...
for (let index = 0; index < rows.length; index += 1) {
  ...
  const expectedId = `B2-EVAL-${String(index + 1).padStart(3, "0")}`;
  ...
  seenIds.add(row.id);
  if (row.level !== "B2" || row.expected_level !== "B2") {
    errors.push(`${label} is not bound to B2.`);
  }
  ...
  if (
    ...
    feedback.short_explanation !== feedback.short_explanation.trim() ||
...
const requiredAstralOffsetCase = rows.find(
  (row) => row.id === "B2-EVAL-075",
);
...
if (
  ...
  errors.push("B2-EVAL-075 must place an astral code point before its edit span.");
}
const report = {
  ok: errors.length === 0,
  errors,

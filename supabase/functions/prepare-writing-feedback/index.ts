...
function validateFeedbackPayload(value: unknown): FeedbackPayload {
  ...
  for (const value of Object.values(parsedScore)) {
    if (!Number.isInteger(value) || value < 0) throw new Error("Invalid score summary counts.");
  }
...
Deno.serve(async (req) => {
  ...
  if (submissionError) {
    ...
    return jsonResponse({ error: "Could not load submission." }, 500);
  }
  if (!submission) {
  ...
  if (!isPlatformAdmin) {
    const { data: membership } = await admin
      ...
      .maybeSingle();
    isWorkspaceTeacher = Boolean(membership);
  }

  if (!isPlatformAdmin && !isWorkspaceTeacher) {
    return jsonResponse({ error: "Permission denied." }, 403);
  }

  const originalText = cleanString(submission.original_text);
  if (!originalText) {
    return jsonResponse({ error: "Submission text is empty." }, 400);
  }
  if (originalText.length > 12000) {
    return jsonResponse({ error: "Submission text is too long." }, 400);
  }
  const lineCount = originalText.split(/\r?\n/).filter((line) => line.trim()).length;
  if (lineCount > 120) {
    return jsonResponse({ error: "Submission has too many lines for feedback." }, 400);
  }
  if (submission.status === "draft") {
    return jsonResponse({ error: "Draft submissions cannot be checked." }, 400);
  }
  if (submission.status === "checked") {
    return jsonResponse({ error: "Feedback already exists for this submission." }, 409);
  }
  if (submission.status === "checking") {
    return jsonResponse({ error: "Feedback is already being prepared." }, 409);
  }
  ...
  const { data: lockedSubmission, error: lockError } = await admin
    ...
    .eq("id", submissionId)
    .in("status", ["submitted", "needs_review", "failed"])
    .select("*")
    .maybeSingle();

  if (lockError) {
    console.error("prepare-writing-feedback lock failed", lockError.message);
    return jsonResponse({ error: "Could not start feedback preparation." }, 500);
  }
  if (!lockedSubmission) {
    return jsonResponse({ error: "Submission is not ready for feedback." }, 409);
  }
  ...
  if (!apiKey) {
    await admin
      ...
      .eq("id", submissionId);
    return jsonResponse({ error: "Feedback service secret is not configured." }, 503);
  }
  ...
  try {
    ...
    if (!deepseekResponse.ok) {
      console.error("prepare-writing-feedback deepseek failed", deepseekResponse.status);

-- Preserve truthful AI-spend evidence when the isolated worksheet-live browser
-- canary is removed. The detached ledger remains content-free and immutable;
-- this migration only distinguishes the two exact synthetic cleanup sources.

alter table app_private.ai_canary_spend_archive
  drop constraint if exists ai_canary_spend_archive_archive_source_check;

alter table app_private.ai_canary_spend_archive
  alter column archive_source set default 'writing_live_canary_cleanup',
...
create index ai_canary_spend_archive_fixture_run_idx
...
create function app_private.archive_worksheet_live_canary_spend(
    ...
    or exists (
      ...
      where assignment.id = target_bank_assignment_id
        and (
          ...
          or assignment.worksheet_level is distinct from 'A2'
...
revoke all on function app_private.archive_worksheet_live_canary_spend(
  uuid, text, uuid, uuid, uuid
) from public, anon, authenticated, service_role;

create function public.archive_worksheet_live_canary_spend(
  target_workspace_id uuid,
  target_workspace_slug text,
  target_batch_id uuid,
  target_provider_assignment_id uuid,
...
begin
  ...
  return query
  select *
  from app_private.archive_worksheet_live_canary_spend(
    target_workspace_id,
...
revoke all on function public.archive_worksheet_live_canary_spend(
  uuid, text, uuid, uuid, uuid
) from public, anon, authenticated, service_role;
grant execute on function public.archive_worksheet_live_canary_spend(
  uuid, text, uuid, uuid, uuid
) to service_role;

create function api.archive_worksheet_live_canary_spend(
  target_workspace_id uuid,
  target_workspace_slug text,
  target_batch_id uuid,
  target_provider_assignment_id uuid,
...
as $$
  select *
  from public.archive_worksheet_live_canary_spend(
    target_workspace_id,
    target_workspace_slug,
    target_batch_id,
    target_provider_assignment_id,
...
revoke all on function api.archive_worksheet_live_canary_spend(
  uuid, text, uuid, uuid, uuid
) from public, anon, authenticated, service_role;
grant execute on function api.archive_worksheet_live_canary_spend(
  uuid, text, uuid, uuid, uuid
) to service_role;

comment on table app_private.ai_canary_spend_archive is
  'Content-free immutable spend evidence detached only for exact synthetic writing-live or worksheet-live fixtures; never used for normal tenant cleanup.';
comment on column app_private.ai_canary_spend_archive.archive_run_id is
  'Opaque per-run linkage for the one deterministic worksheet-live canary; null for the unchanged writing-live archive contract.';
comment on function app_private.archive_worksheet_live_canary_spend(
  uuid, text, uuid, uuid, uuid
) is
  'Private fail-closed atomic copy-before-delete implementation for terminal spend in one deterministic worksheet-live staging fixture.';
comment on function api.archive_worksheet_live_canary_spend(
  uuid, text, uuid, uuid, uuid
) is
  'Service-only invoker facade for exact terminal worksheet-live canary spend archival; no browser role can execute it.';

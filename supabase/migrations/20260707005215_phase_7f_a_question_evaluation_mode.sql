-- Phase 7F-A: mark flexible worksheet questions for the existing Phase 7D-2 evaluator.
-- This keeps local scoring for deterministic questions while routing flexible text answers
-- through stored post-submit reviews.

alter table public.practice_test_questions
  add column if not exists evaluation_mode text not null default 'local_exact';
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'practice_test_questions_evaluation_mode_check'
      and conrelid = 'public.practice_test_questions'::regclass
  ) then
    alter table public.practice_test_questions
      add constraint practice_test_questions_evaluation_mode_check
      check (evaluation_mode in ('local_exact', 'open_evaluation'));
  end if;
end;
$$;

update public.practice_test_questions
set evaluation_mode = 'open_evaluation'
where question_type in ('mini_writing')
  or lower(btrim(coalesce(correct_answer, ''))) in (
    'manual_review',
    'manual review',
    'open_review',
    'flexible_review',
...
set search_path = public, pg_temp
as $$
  select app_private.is_practice_question_locally_scorable(question_type, correct_answer, 'local_exact');
$$;

revoke all on function app_private.is_practice_question_locally_scorable(text, text, text) from public, anon, authenticated;
revoke all on function app_private.is_practice_question_locally_scorable(text, text) from public, anon, authenticated;

create or replace function app_private.submit_practice_attempt_internal_phase_7d2_unchecked(
...
declare
  ...
  scoring_version_value text := 'phase_7f_a_evaluation_mode_v1';
begin
  ...
  if exists (
    ...
    where jsonb_typeof(answer_item) <> 'object'
      or not ((answer_item ->> 'question_id') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$')
  ...
  scorable_questions as (
    ...
    where app_private.is_practice_question_locally_scorable(aq.question_type, aq.correct_answer, aq.evaluation_mode)
  ),
  classified as (
    select
      sq.id,
...
begin
  ...
  questions as (
    select
      ...
      ptq.correct_answer,
      ptq.evaluation_mode,
      app_private.is_practice_question_locally_scorable(ptq.question_type, ptq.correct_answer, ptq.evaluation_mode) as locally_scorable,
  ...
  set
    ...
    scoring_version = 'phase_7f_a_local_plus_open_evaluation_mode_v1',
    feedback = jsonb_build_object(
      'scoring_version', 'phase_7f_a_local_plus_open_evaluation_mode_v1',
      'scoring', scoring_mode,
      'total_questions', total_question_count,
...
begin
  ...
  return query
    with answer_map as (
      ...
      from jsonb_array_elements(attempt_record.answers) answer_item
      where jsonb_typeof(answer_item) = 'object'
        and (answer_item ->> 'question_id') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
      order by (answer_item ->> 'question_id')::uuid
    ),
    questions as (
      select

-- Phase 7C: secure post-submit worksheet review and safer local scoring.
-- Pre-submit question loading stays answer-safe through get_practice_assignment_questions.

create or replace function app_private.is_practice_topic_strict_scoring(
  topic_name text,
  topic_slug text
)
returns boolean
...
as $$
  select (coalesce(topic_name, '') || ' ' || coalesce(topic_slug, '')) ~*
    '(capital|spelling|rechtschreib|orthograph)';
$$;

revoke all on function app_private.is_practice_topic_strict_scoring(text, text) from public, anon, authenticated;

create or replace function app_private.normalize_practice_answer(
  answer_value text,
...
as $$
  ...
  select case
    when strict_scoring then value
    else lower(value)
  end
  from normalized;
$$;

revoke all on function app_private.normalize_practice_answer(text, boolean) from public, anon, authenticated;
...
set search_path = public, pg_temp
as $$
  select app_private.normalize_practice_answer(submitted_answer, strict_scoring)
    = app_private.normalize_practice_answer(correct_answer, strict_scoring);
$$;
...
begin
  ...
  scored as (
    select
      sq.id,
      case
        when app_private.practice_answer_is_correct(submitted.answer, sq.correct_answer, strict_scoring)
        then 1
        else 0
      end as is_correct
  ...
  set
    ...
    feedback = jsonb_build_object(
      ...
      'scoring_normalization', case
        ...
        else 'case_insensitive_trim_spaces_ignore_final_punctuation'
      end
    )
...
begin
  ...
  return query
    ...
    select
      ...
      case
        when not q.locally_scorable then 'submitted_for_review'
        when app_private.practice_answer_is_correct(q.student_answer, q.correct_answer, strict_scoring) then 'correct'
        else 'incorrect'

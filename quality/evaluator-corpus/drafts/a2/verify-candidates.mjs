...
// Severity is intentionally pedagogical rather than a proxy for whether a
// sentence is grammatical. Clause-order repairs are major; local morphology,
// case, preposition, agreement, spelling, and whitespace repairs are minor.
const majorIssueCaseNumbers = new Set([
  15, 18, 20, 25, 28, 30, 57, 64, 80, 103, 108, 118, 120, 136,
]);
...
for (let index = 0; index < rows.length; index += 1) {
  ...
  const expectedId = `A2-EVAL-${String(index + 1).padStart(3, "0")}`;
  ...
  seenIds.add(row.id);
  if (row.level !== "A2" || row.expected_level !== "A2") {
    errors.push(`${label} is not bound to A2.`);
  }
  ...
  const expectedIssueStatus = majorIssueCaseNumbers.has(index + 1)
    ? "major_issue"
    : "minor_issue";
  if (
    feedback.changes.length > 0 &&
    feedback.line_status !== expectedIssueStatus
  ) {
    errors.push(
      `${label} must use ${expectedIssueStatus} under the A2 severity rubric.`,
    );
  }
  if (majorIssueCaseNumbers.has(index + 1) && feedback.changes.length === 0) {
    errors.push(`${label} is a required major clause-order repair.`);
  }
  if (
    ["correct", "acceptable_for_level"].includes(feedback.line_status) &&
    row.expected_topic_slug !== null
  ) {
    errors.push(`${label} assigns a topic to a correct or acceptable line.`);
  }
  if (
    ["minor_issue", "major_issue"].includes(feedback.line_status) &&
    row.expected_topic_slug === null
  ...
  if (
    expectedCategory === "missing_space" &&
    row.expected_topic_slug !== "spelling"
  ) {
    errors.push(`${label} must map the missing space to spelling.`);
  }
  if (
...
const requiredAstralOffsetCase = rows.find((row) => row.id === "A2-EVAL-075");
const requiredAstralChange =
  requiredAstralOffsetCase?.expected_feedback?.changes?.[0];
if (
  !requiredAstralChange ||
  !/[\u{10000}-\u{10ffff}]/u.test(
    unicodeSlice(
      requiredAstralOffsetCase.input_text,
      0,
      requiredAstralChange.source_start,
    ),
  )
) {
  errors.push(
    "A2-EVAL-075 must place an astral code point before its edit span.",
  );
}
const rowsById = new Map(rows.map((row) => [row.id, row]));
const case059 = rowsById.get("A2-EVAL-059");
if (
  case059?.expected_feedback?.corrected_text !== case059?.input_text ||
  case059?.expected_feedback?.line_status !== "acceptable_for_level" ||
  case059?.expected_feedback?.changes?.length !== 0 ||
  case059?.expected_topic_slug !== null
) {
  errors.push(
    "A2-EVAL-059 must remain an acceptable no-change ambiguity case.",
  );
}
const case048 = rowsById.get("A2-EVAL-048");
if (case048?.expected_topic_slug !== "subject-verb-agreement") {
  errors.push(
    "A2-EVAL-048 must map the first-person finite-verb repair to subject-verb-agreement.",
  );
}
const case065 = rowsById.get("A2-EVAL-065");
if (case065?.expected_topic_slug !== "modal-verbs") {
  errors.push(
    "A2-EVAL-065 must map the infinitive required by kann to modal-verbs.",
  );
}
const case067 = rowsById.get("A2-EVAL-067");
if (case067?.expected_topic_slug !== "subject-verb-agreement") {
  errors.push(
    "A2-EVAL-067 must map the singular-subject finite-verb repair to subject-verb-agreement.",
  );
}
const case072 = rowsById.get("A2-EVAL-072");
if (
  case072?.input_text !== "„Natürlich“, sagt er. „Ich helfe du gern.“" ||
  case072?.expected_feedback?.corrected_text !==
    "„Natürlich“, sagt er. „Ich helfe dir gern.“" ||
  JSON.stringify(case072?.expected_feedback?.changes) !==
    JSON.stringify([
      { source_start: 33, source_end: 35, from: "du", to: "dir" },
    ])
) {
  errors.push("A2-EVAL-072 must keep its unambiguous direct-speech fixture.");
}
const case077 = rowsById.get("A2-EVAL-077");
if (case077?.expected_topic_slug !== "articles") {
  errors.push(
    "A2-EVAL-077 must map possessive-article inflection to articles.",
  );
}
const report = {
  ok: errors.length === 0,
